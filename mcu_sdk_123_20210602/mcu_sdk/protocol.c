...
void all_data_update(void)
{
    ...
    mcu_dp_raw_update(DPID_DREAMLIGHT_SCENE_MODE,当前炫彩情景指针,当前炫彩情景数据长度); //RAW型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_dreamlight_scene_mode_handle
功能描述 : 针对DPID_DREAMLIGHT_SCENE_MODE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_dreamlight_scene_mode_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_raw_update(DPID_DREAMLIGHT_SCENE_MODE,value,length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_DREAMLIGHT_SCENE_MODE:
            //炫彩情景处理函数
            ret = dp_download_dreamlight_scene_mode_handle(value,length);
        break;
